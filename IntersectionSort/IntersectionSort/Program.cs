...
namespace IntersectionSort
{
    class Program
    {
        ...
        public static void IntersectionSort()
        {
            for(int j = 1; j < roster_dat.Count; j++)
            {
                Employee key = roster_dat[j];
                int i = j - 1;
                while(i >= 0 && roster_dat[i].getID() > key.getID())
                {
                    cp++;
                    roster_dat[i + 1] = roster_dat[i];
                    ex++;
                    i = i - 1;
                }
                roster_dat[i + 1] = key;
                ex++;
                foreach (Employee ep in roster_dat)
                {
